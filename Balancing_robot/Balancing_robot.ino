...
float pid_i_gain = 0*.02;                                      //Gain setting for the I-controller (1.5)
...
void loop(){
  ...
  */
  ...
  // My angle accel calculation
  Wire.beginTransmission(gyro_address);                                     //Start communication with the gyro
  Wire.write(0x3D);                                                         //Start reading at register 3F
  Wire.endTransmission();                                                   //End the transmission
  Wire.requestFrom(gyro_address, 4);                                        //Request 2 bytes from the gyro
  ay = Wire.read()<<8|Wire.read();                                          //Combine the two bytes to make one integer
  az = Wire.read()<<8|Wire.read();

  angle_acc = (90.0-atan2((float)ay,(float)az)* 57.296);           //Calculate the current angle according to the accelerometer
  ...
  Wire.beginTransmission(gyro_address);                                     //Start communication with the gyro
  Wire.write(0x43);                                                         //Start reading at register 43 (reads gx, g
  Wire.endTransmission();                                                   //End the transmission
  ...
  angle_gyro = angle_gyro * 0.996 + angle_acc * 0.004;  //Correct the drift of the gyro angle with the accelerometer angle
  //angle_gyro = angle_gyro * 1.0 + angle_acc * 0.0;  //Correct the drift of the gyro angle with the accelerometer angle
  //angle_gyro = angle_gyro * 0.9 + angle_acc * 0.1;                    //Correct the drift of the gyro angle with the accelerometer angle
...
*/
  ...
  if(pid_setpoint == 0){                                                    //If the setpoint is zero degrees
    if(pid_output < 0)self_balance_pid_setpoint += 0*0.0015;                  //Increase the self_balance_pid_setpoint if the robot is still moving forewards
    if(pid_output > 0)self_balance_pid_setpoint -= 0*0.0015;                  //Decrease the self_balance_pid_setpoint if the robot is still moving backwards
  }
...
ISR(TIMER2_COMPA_vect){
  ...
  //else if(throttle_counter_left_motor == 1)PORTD |= 0b00010000;             //Set output 4 high to create a pulse for the stepper controller
  //else if(throttle_counter_left_motor == 2)PORTD &= 0b11101111;             //Set output 4 low because the pulse only has to last for 20us 
  ...
  if(throttle_counter_right_motor > throttle_right_motor_memory){           //If the number of loops is larger then the throttle_right_motor_memory variable
    ...
    if(throttle_right_motor_memory < 0){                                    //If the throttle_right_motor_memory is negative
      PORTD |= 0b00000100;                                                  //Set output 2 low to reverse the direction of the stepper controller
      throttle_right_motor_memory *= -1;                                    //Invert the throttle_right_motor_memory variable
    }
    else PORTD &= 0b11111011;                                               //Set output 2 high for a forward direction of the stepper motor
  }
